...
func main() {
	...
	num := make([]int, 0) // Для большей наглядности RWMutex решил увеличить размер слайса
...
func CubeSumWithMutex(in []int) int {
	sliceCopy := append(in[:0:0], in...) // Создаём слайс, который является копией входного слайса поскольку без копии мы бы вносили изменения в изначальный слайс num из main ибо в слайсах используются ссылки
	var CubeSum int
	...
	for i, v := range sliceCopy {
		wg.Add(1) // Инкриментим счётчик для отслеживания количества запущенных горутин
		go func(index, v int) {
	...
	wg.Wait() // Ожидаем выполнение всех горутин - пока wg.Add не приобретет значение 0
	return CubeSum
}
...
func CubeSumWithChan(in []int) int {
	...
	// Объявляем WaitGroup для синхронизации горутин
	var wg sync.WaitGroup
	go func() { // Запускаем горутину, которая бесконечно читает функции из канала ch и выполняет их до момента пока wg.Wait не завершит её
		for {
			f := <-ch
			f()
		}
